...
export const BlogFilter = ({ categories, posts }: BlogFilterProps) => {
    ...
    const activeCategory = pathname === '/grid-1/blog-one' ? 'all' : pathname.split('/grid-1/blog-one/category/')[1]?.split('/')[0] || 'all'

    const handleClick = (slug: string) => {
        if (slug === 'all') router.push('/grid-1/blog-one')
        else router.push(`/grid-1/blog-one/category/${slug}`)
    }
